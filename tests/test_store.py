...
class TestCrypto:
    def test_encrypt_decrypt_roundtrip(self, cipher: RecordCipher) -> None:
        ...
        blob = cipher.encrypt(CRYPTO_KEY, data)
        assert blob != data
        assert cipher.decrypt(CRYPTO_KEY, blob) == data

    def test_decrypt_wrong_record_key_raises(self, cipher: RecordCipher) -> None:
        blob = cipher.encrypt(CRYPTO_KEY, b"data")
    ...
    def test_decrypt_corrupted_blob_raises(self, cipher: RecordCipher) -> None:
        ...
        with pytest.raises(ValueError):
            cipher.decrypt(CRYPTO_KEY, corrupted)
...
class TestMemoryStore:
    ...
    async def test_put_if_absent_keeps_first(
        ...
        other = MappingRecord(original_fp="zzz", masked_text=OTHER_TEXT, replacements=())
    ...
    async def test_put_overwrites_and_sets_ttl(
        self, cipher: RecordCipher, record: MappingRecord
    ) -> None:
        now = 1000.0
        store = MemoryStore(cipher, clock=lambda: now)
        other = MappingRecord(original_fp="zzz", masked_text=OTHER_TEXT, replacements=())
        await store.put("k", record, ttl_seconds=10)
        await store.put("k", other, ttl_seconds=10)
        assert await store.get("k") == other
        now += 11
...
class TestRedisStore:
    ...
    async def test_put_overwrites_and_sets_ttl(
        ...
        other = MappingRecord(original_fp="zzz", masked_text=OTHER_TEXT, replacements=())
        await redis_store.put("k", record, ttl_seconds=60)
        await redis_store.put("k", other, ttl_seconds=60)
        assert await redis_store.get("k") == other
        ttl = await redis_store._client.ttl(REDIS_KEY)
        assert ttl > 0
    ...
    async def test_raw_bytes_do_not_leak_original(
        ...
        await redis_store.put_if_absent("k", record, ttl_seconds=60)
        raw = await redis_store._client.get(REDIS_KEY)
...
class TestFailoverStore:
    async def test_fallback_on_connection_error(self, record: MappingRecord) -> None:
        ...
        assert store.backend == DEGRADED
        assert fallback.calls == 1
        assert await store.get("k") == record
        assert fallback.calls == 2

    async def test_put_falls_back_on_connection_error(self, record: MappingRecord) -> None:
        now = 0.0
        primary = _FakeStore(fail=True)
        fallback = _FakeStore()
        store = FailoverStore(primary, fallback, retry_after_seconds=5.0, clock=lambda: now)

        await store.put("k", record, ttl_seconds=60)
        assert store.backend == DEGRADED
        assert fallback.calls == 1
        assert await store.get("k") == record
        assert fallback.calls == 2
